...
export default class Studienplan {
  ...
  constructor(schwerpunkt = '', ps = {
    ...
    if (generatePStunden) {
      this.generatePStunden();
    }
  }

  /**
   * Generates p-stunden from p1-p5
   */
  generatePStunden() {
    const ps = {
      p1: this.p1,
    ...
    for (let i = 1; i < 6; i += 1) {
      const p = `p${i}`;
      if (!_.isEmpty(ps[p]) && !this.fachstunden[ps[p]]) {
    ...
    switch (this.schwerpunkt) {
      ...
      case 'spr': {
        if (this.p2 === 'de' && !(c.isFremdsprache(this.p3)
          ...
          return {
            faecher: this.filterExisting(c.fremdsprachen),
      ...
      case 'nw': {
        if (this.p2 === 'ma' && !(c.isNW(this.p3) || c.isNW(this.p4) || c.isNW(this.p5))) {
          return {
            faecher: this.filterExisting(c.naturwissenschaften),
            belegung: c.defaultBelegung,
          };
        }
        return { faecher: null };
      ...
      case 'mk': {
        ...
        if (this.p1 === 'mu') {
          return { faecher: this.filterExisting(['ku', 'ds']), belegung };
        }
        return { faecher: this.filterExisting(['mu']), belegung };
      }
      default: {
        return { faecher: null };
      }
  ...
  /**
   * Gets array of faecher, the student can and must choose from
   * @return {Array} array of objects with faecher (as object) and belegung as keys
   */
  getChooseableFaecher() {
    const cFaecher = [];
    c.chooseFaecher.forEach((facharr) => {
      if (!this.hasArrayFach(c.fachToKz(facharr.faecher))) {
        cFaecher.push(facharr);
      }
    });
    /*
     Sonderfall GW:
     EF-Option 2 FS: 2 Fremdsprachen erzwingen
     EF-Optionen 2 NW: 2 Naturwissenschaften erzwingen
     */
    if (this.schwerpunkt === 'gw' && this.countAppearance(c.naturwissenschaften) < 2 && this.countAppearance(c.fremdsprachen) < 2) {
      const ef = this.getEF();
      if (c.naturwissenschaften.includes(ef)) { // 2 Naturwissenschaften
        cFaecher.push({
          faecher: c.kzToFach(this.filterExisting(c.naturwissenschaften)),
          belegung: c.parseBelegungAlts('4x4'),
        });
      } else { // 2 Fremdsprachen
        cFaecher.push({
          faecher: c.kzToFach(this.filterExisting(c.fremdsprachen)),
          belegung: c.parseBelegungAlts('4x4'),
        });
      }
    }
  ...
  /**
   * Returns faecher (kzs(!)) which can be choosen additionally.
   * @return {String[]} Array of kzs
   */
  getZusatzFaecher() {
    return this.filterExisting(Object.keys(c.faecher));
  }
  ...
  /**
   * Removes existing faecher (p1-p5, fachstunden) from array
   * @param arr Array containing kzs
   * @return {*|{start}} array without p1-p5
   */
  filterExisting(arr) {
    return _.difference(arr,
      _.concat(_.keys(this.fachstunden), this.p1, this.p2, this.p3, this.p4, this.p5),
    );
  }
