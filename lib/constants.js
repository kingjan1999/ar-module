...
// Fächer, die belegt werden müssen, ohne Wahlmöglichkeit zwischen z.B. verschiedenen Sprachen
export const defaultFaecher = [
  {
    fach: faecher.de,
    belegung: defaultBelegung,
  }, {
    fach: faecher.ma,
    belegung: defaultBelegung,
  }, {
    fach: faecher.ge,
    belegung: parseBelegungAlts('2x2x11|2x2x12|2x4'),
  },
  {
    fach: faecher.po,
    belegung: parseBelegungAlts('2x2x12|2x2x11|2x4'),
  }, {
    fach: faecher.sf,
    belegung: parseBelegungAlts('4x2'),
  }, {
    fach: faecher.sp,
    belegung: parseBelegungAlts('4x2'),
  },
...
export const chooseFaecher = [
  {
    faecher: [faecher.mu, faecher.ku, faecher.ds],
    belegung: parseBelegungAlts('2x2x11|2x2x12|4x2'),
  },
  {
    faecher: [faecher.er, faecher.kr, faecher.wn],
    belegung: parseBelegungAlts('4x2'),
  },
  {
    faecher: kzToFach(fremdsprachen),
    belegung: parseBelegungAlts('4x4'),
  },
  {
    faecher: kzToFach(naturwissenschaften),
    belegung: parseBelegungAlts('4x4'),
  },
];
...
/**
 * Resolves kuerzel to fach
 * @param kz Either array or string containing kz(s)
 * @return {*} Array / Object of faecher / fach
 */
export function kzToFach(kz) {
  if (Array.isArray(kz)) {
    const facharr = [];
    kz.forEach((fach) => {
      facharr.push(faecher[fach]);
    });
    return facharr;
  }
  return faecher[kz];
...
/**
 * Resolves fach to kuerzel
 * @param fach Fach or array of faechers
 * @return {Array} Array / String containing kz(s)
 */
export function fachToKz(fach) {
  if (Array.isArray(fach)) {
    const kzarr = [];
    fach.forEach((kz) => {
      kzarr.push(kz.kz);
    });
...
export function parseBelegung(belegung) {
  switch (belegung) {
    case '4x4': {
      return {
        stunden111: 4,
