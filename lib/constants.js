...
export const fremdsprachen = ['en', 'la', 'sn', 'fr'];
...
export function getLKnachSP(schwerpunkt, pfach) {
  ...
  if (vfaecher === '*') {
    vfaecher = _.chain(faecher)
      .filter(fach => fach.stunden.e4 === true && !keinP.includes(fach.kz))
      .map(fach => fach.kz).value();
  }
...
export function nameToKZ(name) {
  return _.findWhere(faecher, { name }).kz;
}
...
export function getP4s() {
  return _.chain(faecher)
    .filter(fach => !keinP.includes(fach.kz))
    .filter(fach => !nurp5.includes(fach.kz))
    .map(fach => fach.kz)
    .value();
}
...
export function getForFeld(feld) {
  return _.filter(faecher, fach => fach.feld === feld);
}
